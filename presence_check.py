...
# This one cost him a working assistant. The conversation loop stops the
# microphone before it speaks, and only starts it again once the reply is
...
# because the reply never finished, so the mic was never turned back on.
import sys as _sys
...
real_phone = _sys.modules.get("tars_phone")
real_gaming = highlights._gaming
try:
    _sys.modules["tars_phone"] = fake_phone
    ...
    if real_phone is not None:
        _sys.modules["tars_phone"] = real_phone
    else:
        _sys.modules.pop("tars_phone", None)
