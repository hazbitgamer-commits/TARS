...
Opens EA Sports FC 26's Ultimate Team Web App in the browser, and — if the owner names a
player — copies that player's name to the clipboard so it's one paste away from the
in-app search bar.

## Why it's scoped this way

the owner asked TARS to "learn how to use the web app remotely" and "buy players / notify
...
1. **No safe automation path exists.** The FUT Web App is EA's login-gated single-page
   ...
   or selling. Anything that logged in and clicked "buy" on the owner's behalf would risk
...
2. **The hard rule.** TARS is never allowed to spend money or make purchases. Buying
   ...
   never click "buy" itself. the owner always makes that click.

So the skill does the safe, genuinely useful part: get the owner to the right page fast,
...
- Can't log in, buy, or sell — the owner's own login and his own clicks, every time.
